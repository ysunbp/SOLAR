"""
LRU Solver — plugs the LRU Agent into the MemoryBench evaluation framework.

Same retrieval as Embedder, but with LRU eviction when memory is full.
"""

from tqdm import tqdm
from typing import List, Dict

from src.agent.lru import LRUAgent, LRUAgentConfig
from src.solver.base import BaseSolver


class LRUSolver(BaseSolver):
    AGENT_CLASS = LRUAgent

    def __init__(self, config: LRUAgentConfig, memory_cache_dir: str):
        super().__init__(config, memory_cache_dir)
        self.method_name = "lru"
    ...
    def memory_locomo_conversation(self, conversation, session_cnt: int):
        """Store Locomo conversation turns with LRU eviction."""
        pbar = tqdm(total=session_cnt, desc="[LRU] Adding conversation to memory")
        stored_total = 0
        session_idx = 1
        while f"session_{session_idx}" in conversation:
            ...
            for turn in session:
                ...
                self.agent.add_memory_lru(content, doc_id=turn_date_time)
                stored_total += 1
                self.current_conversation_memory_ids.append(turn_date_time)
            session_idx += 1
            pbar.update(1)
        pbar.close()
        metrics = self.agent.get_lru_metrics()
        print(f"  [LRU] Stored {stored_total} turns, "
