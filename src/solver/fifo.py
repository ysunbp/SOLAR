"""
FIFO Solver — plugs the FIFO Agent into the MemoryBench evaluation framework.

Same retrieval as Embedder, but with FIFO eviction when memory is full.
"""

from tqdm import tqdm
from typing import List, Dict

from src.agent.fifo import FIFOAgent, FIFOAgentConfig
from src.solver.base import BaseSolver


class FIFOSolver(BaseSolver):
    AGENT_CLASS = FIFOAgent

    def __init__(self, config: FIFOAgentConfig, memory_cache_dir: str):
        super().__init__(config, memory_cache_dir)
        self.method_name = "fifo"
    ...
    def memory_locomo_conversation(self, conversation, session_cnt: int):
        """Store Locomo conversation turns with FIFO eviction."""
        pbar = tqdm(total=session_cnt, desc="[FIFO] Adding conversation to memory")
        stored_total = 0
        session_idx = 1
        while f"session_{session_idx}" in conversation:
            ...
            for turn in session:
                ...
                self.agent.add_memory_fifo(content, doc_id=turn_date_time)
                stored_total += 1
                self.current_conversation_memory_ids.append(turn_date_time)
            session_idx += 1
            pbar.update(1)
        pbar.close()
        metrics = self.agent.get_fifo_metrics()
        print(f"  [FIFO] Stored {stored_total} turns, "
